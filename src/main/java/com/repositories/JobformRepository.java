...
public interface JobformRepository extends CrudRepository<JobForm, Long> {
    ...
    @Query(value = "SELECT count(work) FROM jobform where work = 'montaz' and  action = 'Izpildits' and work_date like %:date2%", nativeQuery = true)
    String getStatistics(String date2);

    @Query(value = "SELECT count(work) FROM jobform where work = 'remont' and  action = 'Izpildits' and work_date like %:date2%", nativeQuery = true)
    String getRemont(String date2);

    @Query(value = "SELECT count(work) FROM jobform where work = 'snjatie' and  action = 'Izpildits' and work_date like %:date2%", nativeQuery = true)
    String getSnjatie(String date2);

    @Query(value = "SELECT count(sistem) FROM jobform where sistem = 'minibasic' " +
            "and  action = 'Izpildits' and work = 'montaz'and work_date like %:date2%", nativeQuery = true)
